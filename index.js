...
const SIG_SHEETS = ['SIGINT', 'QuantSIG', 'CloudSIG', 'Tardis', 'GameDevSIG', 'TypeSIG', 'ProjectShare', 'EdinburghAI', 'CCSIG', 'BitSIG', 'EVP', 'NeuroSIG'];
const OTHER_SHEETS = ['HTB', 'InfBall', 'Social', 'Misc', 'STMU', 'Subscriptions', 'Merch', 'AoC'];
...
function diffRows(prevRows, currRows) {
  ...
  for (const row in prevRows) {
    ...
    old.get(k).push({ row: row, reason: prevRows[row].reason, amount: prevRows[row].amount });
  }
  ...
  for (const row in currRows) {
    ...
    if (list && list.length > 0) {
      ...
      added.push({ row: row, reason: currRows[row].reason, amount: currRows[row].amount });
    }
  }
